...
import 'customer_schema_loader.dart';
...
class ProductBootstrap {
  ...
  static ProductBootstrap fromJson(Map<String, Object?> json) {
    ...
    return ProductBootstrap(
      ...
      product: readString('product', fallback: 'customer_app'),
      initialScreenId: readString('initialScreenId', fallback: 'customer_home'),
      defaultThemeId: readString(
        'defaultThemeId',
...
class CustomerBootstrapLoader {
  CustomerBootstrapLoader({
  ...
  Future<ConfigSnapshot> loadSnapshot({
    ...
    // Snapshots are immutable-by-ID. If we have a cached body, use it directly.
    if (cache != null) {
      final cached = cache!.readCachedJson('config_snapshot.$snapshotId');
      if (cached != null) {
        return ConfigSnapshot.fromJson(cached);
      }
    }

    final response = await _client.get(uri, headers: headersProvider?.call());
    if (response.statusCode != 200) {
      throw StateError('Snapshot returned ${response.statusCode}');
    }

    final decoded = jsonDecode(response.body);
    if (decoded is! Map) {
      throw const FormatException('Snapshot returned a non-object response');
    }
    ...
    return ConfigSnapshot.fromJson(json);
  }
}
