...
class CustomerDiagnosticsReporter {
  CustomerDiagnosticsReporter({
  ...
  void emitThemeSourceUsed(
    ...
    diagnostics.emit(
      DiagnosticEvent(
        ...
        kind: DiagnosticKind.metric,
        fingerprint:
            '${ThemeLadderEventNames.sourceUsed}:${req.product}:$themeId:$themeMode:${source.wireValue}',
        context: contextForRequest(req, configSnapshotId: configSnapshotId),
        payload: <String, Object?>{
          'themeId': themeId,
          'themeMode': themeMode,
  ...
  void emitThemeFallbackToLocal(
    ...
    diagnostics.emit(
      DiagnosticEvent(
        ...
        kind: DiagnosticKind.metric,
        fingerprint:
            '${ThemeLadderEventNames.fallbackToLocal}:${req.product}:$themeId:$themeMode:${reason.wireValue}',
        context: contextForRequest(req, configSnapshotId: configSnapshotId),
        payload: <String, Object?>{
          'themeId': themeId,
          'themeMode': themeMode,
  ...
  void emitScreenLoadSummary(
    ...
    final schemaFormatVersion =
        bundle.document['schemaVersion'] as String? ?? bundle.schemaVersion;

    diagnostics.emit(
      DiagnosticEvent(
        eventName: ScreenLoadEventNames.summary,
        severity: DiagnosticSeverity.info,
        kind: DiagnosticKind.metric,
        fingerprint:
            '${ScreenLoadEventNames.summary}:${req.product}:${req.screenId}:${finalSchemaSource.wireValue}',
        context: contextForBundle(bundle, req, configSnapshotId),
        payload: <String, Object?>{
          ScreenLoadSummaryKeys.screenLoadId: screenLoadId,
          ScreenLoadSummaryKeys.finalSchemaSource: finalSchemaSource.wireValue,
          if (finalSchemaReason != null)
            ScreenLoadSummaryKeys.finalSchemaReasonCode:
                finalSchemaReason.wireValue,
          if (schemaDocId != null && schemaDocId.isNotEmpty)
            ScreenLoadSummaryKeys.schemaDocId: schemaDocId,
          ScreenLoadSummaryKeys.schemaBundleId: bundle.schemaId,
          ScreenLoadSummaryKeys.schemaBundleVersion: bundle.schemaVersion,
          ScreenLoadSummaryKeys.schemaFormatVersion: schemaFormatVersion,
          ScreenLoadSummaryKeys.parseErrorCount: parseErrorCount,
          ScreenLoadSummaryKeys.refErrorCount: refErrorCount,
          ScreenLoadSummaryKeys.usedRemoteTheme: usedRemoteTheme,
          ScreenLoadSummaryKeys.finalThemeSource: finalThemeSource.wireValue,
          if (themeDocId != null && themeDocId.isNotEmpty)
            ScreenLoadSummaryKeys.themeDocId: themeDocId,
          ScreenLoadSummaryKeys.attemptCount: attemptCount,
          ScreenLoadSummaryKeys.fallbackCount: fallbackCount,
          ScreenLoadSummaryKeys.totalLoadMs: totalLoadMs,
        },
      ),
    );
  }

  static Future<void> saveLkgConfigSnapshotJson({
